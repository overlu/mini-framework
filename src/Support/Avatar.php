...
class Avatar
{
    ...
    public string $LetterFont;
    public string $AsianFont;
    public bool $EnableAsianChar;
    ...
    public function create($Char, $AvatarSize = 256): Avatar
    {
        ...
        $this->LetterFont = $this->fontPath . 'SourceCodePro-Light.ttf';
        $this->AsianFont = $this->fontPath . 'SourceHanSansCN-Normal.ttf';
    ...
    private function _initialize(): Avatar
    {
        ...
        $BackgroundColorIndex = random_int(0, count($MaterialDesignColor) - 1);
     ...
     * @return bool
     */
    public function output2Browser(): bool
    {
        header('Content-Type: image/png');
        return imagepng($this->resize($this->AvatarSize));
    }
     ...
     * @throws \Exception
     */
    public function save($filename = null, $Path = null): string
    {
        $file = ($filename ?? random_int(100, 999) . round(microtime(true))) . '.png';
        if (!$Path) {
            if (!is_dir($this->uploadPath) && !mkdir($concurrentDirectory = $this->uploadPath, 0777, true) && !is_dir($concurrentDirectory)) {
                throw new \RuntimeException(sprintf('Directory "%s" was not created', $concurrentDirectory));
            }
            $Path = $this->uploadPath . DIRECTORY_SEPARATOR . $file;
        }
        imagepng($this->resize($this->AvatarSize), $Path);
        return 'avatar' . DIRECTORY_SEPARATOR . date('Ymd') . DIRECTORY_SEPARATOR . $file;
    }
}